...
When payment-service behavior changes, update `src/BlazorTestApp/Pages/Services/Payment.razor` for all reads. Mock HTTP boundaries in tests and cover each route, method, query/authentication behavior, response parsing, invalid input, cancellation, and DI registration.

## Review API maintenance

Treat the Etsy OpenAPI 3.0.0 specification as the source of truth for all Review Management routes, query parameters, response types, and OAuth scopes. Maintain the complete 2-operation surface: reviews by listing and reviews by shop. Both Review Management operations are public reads: send the `x-api-key` header only and do not add a bearer token unless Etsy adds an operation-level OAuth scope. Keep the service on its named `IHttpClientFactory` client, validate IDs, pagination, and supplied timestamps before issuing a request, encode query parameters safely, and expose cancellation through every asynchronous method.
...
When shipping-service behavior changes, update `src/BlazorTestApp/Pages/Services/Shipping.razor` for all non-destructive reads. Mock HTTP boundaries in tests and cover each route, method, query/form serialization, authentication behavior, response parsing, invalid input, cancellation, and DI registration.
